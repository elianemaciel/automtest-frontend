...
export default function ApplicationBody() {
  ...
  const [selectedMenuOption, setSelectedMenuOption] = useState<MenuButton>(
    MenuButton.ABOUT,
  );
    ...
    >
      ...
      <div style={{ marginInlineStart: '24px', width: '100%' }}>
        {selectedMenuOption === MenuButton.ABOUT ? (
          ...
          <InsertMethodsInfoContent
            methods={methods}
            setMethods={setMethods}
            showEquivClassesList={() =>
              selectButton(MenuButton.EQUIVALENCE_CLASS)
            }
          />
        ) : selectedMenuOption === MenuButton.EQUIVALENCE_CLASS ? (
          <EquivalenceClassesContent
            methods={methods}
            setMethods={setMethods}
            selectedIA={selectedIA}
